...
なお、本記事ではこの言葉の定義はこうであるべきだと強く主張または是正するような意図はありません。しかし、レンダーという言葉の意味や、例えば SSR ってどんな文脈な言葉だったかなどの疑問を解消する助けになれば幸いです。

(筆者は React/Next.js での開発が主な業務であるため、React に偏った内容になってしまっていますがご了承ください。)

## 利用する用語の意味

- **サーバーサイド**: ブラウザなどの Web クライアントに対して、　 Node.js などのランタイムのこと。React のコードの実行やデータフェッチ、HTML ファイルの組み立てなどの処理を行うことができ、もっぱら BFF と区分されるような層のことを指す
- **クライアントサイド**: ブラウザなどの Web クライアント
...
**用語の意味. サーバーサイドで、毎ページリクエスト時に HTML ファイル(とその中身)を生成すること、あるいはそのレンダリングパターン**

**レンダリングの意味. HTML ファイルを生成すること**

レンダリングという言葉の対象は、HTML ファイル(とそのコンテンツ)です。
...
:::message
当時は**同じコード**がサーバーサイドでもクライアントサイドでも実行されることを **Isomolphic**と呼びました。現在でもサーバーサイド、クライアントサイドの両側で実行される概念に対して Isomolphic という言葉が使われることがあります。
:::
...
しかし、現在の SSR という用語は、むしろ**本来はクライアントで動くコード**をサイト訪問時だけ、**敢えてサーバーサイドでレンダリングする**という文脈の言葉に変わってきたと考えることもできるのではないでしょうか。

:::message
Astro と SSR
MPA のフレームワークである Astro でも SSR は On-demand Rendering という概念として存在しており、ページリクエスト時に HTML をサーバーサイドで生成すること という意味になっています。
Astro はもともと Static Site Generation(SSG) に特化して登場したフレームワークで、SPA に対抗して、Rails などと同様 MPA 的な遷移をしながらも JavaScript 以外の言語との二重開発をする必要がないことが利点であるとしています。
SSG の静的ページのみで良ければ、build 時に生成した静的 HTML を配信して、外部 API とのやり取りはブラウザで行えば Node.js 等のランタイムは不要なのですが、On-demand Rendering が追加され、本来持たなかったサーバーサイドランタイムを持つという手段をとることで、クライアントサイドで動的生成していた作業を(CSR)をサーバーサイドで行いコンテンツ入りの HTML ファイルを配信することができるようになりました。
...
よって、SC ではデータ取得を行った上で、レンダリング結果を出すことができ、それらの処理はサーバーサイドのみで完結させることができるのです。
