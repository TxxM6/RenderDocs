...
| 名称                           | 意味                                                            |
| ------------------------------ | --------------------------------------------------------------- |
|**サーバーサイド**| Web クライアント（ブラウザなど）に対して、Node.js などのランタイムのことを指す。データフェッチやReactなどのコードを実行してコンテンツを含んだHTMLを生成・配信する役割を担う。[^サーバーサイド]|
...
[^サーバーサイド]:本記事ではサーバーサイドとビルド時の実行環境は区別しています。本記事のMPAの説明では、サーバーサイドとビルド時を合わせてサーバーという言い方をしています。
