...
- **Static Site Generation(SSG)**
  - ビルド時に HTML (およびそのコンテンツ)を生成すること、あるいはそのレンダリングモデル
- **Partial Prerendering(PPR)**
  - 同一のルート(ページ)内でSSR/SSGのようにレンダリングするコンポーネントを両立するレンダリングモデル

CSRやSSRは、初回表示時や画面遷移時などのページを表示するタイミングに、**ページ　および　コンテンツをどのようにを生成、表示するか**という「レンダリング手法」・「レンダリング技術」を表す用語です。
ページを表示する際には、データの取得やコンテンツ生成といったプロセスがあり、それらをHTMLとして出力したり、DOMを操作することで画面に反映されます。
これらの用語の区別は、簡潔に言えば、このプロセスが実行される場所・タイミングが、クライアントサイドであればCSR、サーバーサイドランタイムであればSSR、ビルド時であればSSGであると言えるでしょう。しかし、SSRの用語の意味や意義は、初期の頃から近年に至るまで変化がありました。
PPRは、同一ページ内においてリクエスト時にサーバーサイドで生成されるコンテンツとビルド時に生成されるコンテンツを共存できるレンダリング手法です。
...
SPAは、アプリケーション全体の動作の仕組みやアーキテクチャを指す用語です。ソフトナビゲーションという遷移方法によって単一のHTML内でページ遷移をする際、ページ全体を再読み込みせず、必要な部分だけを更新するアプリケーションのことを指します。
しかし、時としてSPAという用語は初期のSPAのような **SSRのようなサーバーサイドランタイムを持たない**アプリケーションであるという意味で利用され、SSRなどのレンダリングモデルの用語と同列に扱われ、比較されることも少なくないです。
...
CSRが登場する前の**従来のWebアプリケーション**では、初回表示時やページ遷移時に、サーバー側でデータ取得・ページ生成を行い、完成したHTMLをブラウザに送信し画面全体を更新していました(MPA[^MPA])。
...
CSR のようにクライアントサイドで コンテンツを生成しDOM を直接書き換える方法は当時は一般的ではなく、この頃の Web アプリケーションは、サーバーサイドのテンプレートエンジン(Java,PHP,Ruby on Rails 等)で HTMLを生成していました。この方法では、初回表示もナビゲーション時もサーバーサイドでコンテンツを含む HTML を生成し、ブラウザでは HTML 全体を入れ替える形で画面を更新します(MPA[^MPA])。SPAの普及以降、CSRと対比してこの従来の手法がSSRと呼ばれるようになりました。
...
:::message
Astro と SSR
ここまでモダンなフレームワークにおけるSSRという用語が基本的にソフトナビゲーションとセットになるかのように述べてきました。
MPA[^MPA] のフレームワークである Astro でも SSR は [On-demand Rendering](https://docs.astro.build/en/guides/on-demand-rendering/) という機能として存在しており、ページリクエストに応じてサーバーサイドで HTML を生成する仕組みを指します。 
Astro は [Static Site Generation(SSG)](#static-site-generation(ssg)(%E5%88%A5%E7%A7%B0%3A-pre-rendering)) に特化したフレームワークとして登場し、従来の MPA(Ruby on Railsなど) のようにページ遷移を行いながらも、JavaScript 以外の言語との二重開発が不要であることを利点としています。
全てのページにおいて静的 HTML をビルド時に生成し、動的なコンテンツはクライアントサイドで外部 API からデータ取得してきて画面を更新すれば、 Node.js 等のサーバーサイドランタイムは不要です。
しかしOn-demand Rendering が追加され、新たにサーバーサイドランタイムを持つことで、これまでクライアントサイドで行っていた動的なコンテンツ生成 (CSR) をサーバーサイドで行い、コンテンツを含む HTML ファイルを配信できるようになりました。(そして、Astroの Client Islandsと呼ばれる部分はハイドレーションされます。)
よって Astro の On-demand Rendering が解決した問題は MPAでありながらも、Next.Js などの SSR with Hydration と多くの点で共通していたと筆者は考えています。　
:::
...
### 事前レンダリング
**Pre-Rendering**という呼び方をそのまま日本語に直すと **事前レンダリング** となりますが、この言葉はSSR、SSGをまとめて指す言葉として少なからず使われてきました。
クライアントサイドにレスポンスが返却されるよりも前の段階でレンダリングされるという意味合いであると捉えることができそうです。
...
筆者が観測する限り大きく分けて二つの意味で使用されているようですが、いずれの場合もソフトナビゲーションで画面遷移する点では共通しているようです。この二つの用例の間では「単一のページ」の捉え方に差があるように感じています。
CSR/SSR/SSGをSPAを構築するレンダリング技術であるという解釈がある一方で、 SPAとSSRを対比させるケースも見られます。
...
初回表示もCSRの場合、SSRした時などとは異なり、アプリケーション全体を通して、どのルート(ページ)においても共通のHTMLファイルが配信されるため、アプリケーション全体が単一のページで構成されていると言えるかもしれません。しかし、こちらの意味の場合でも、ハードナビゲーションではなく、ページ内を部分的に更新するような遷移をすることも条件とされていそうです。
...
この場面において SPAとSSR、SSGとの争点は、初回アクセス時に配信するコンテンツ(HTML)の表示方法です。SPAの意味は **初回表示をCSRで行うアプリケーション**というニュアンスになります。(そして、この文脈においては、SPAの代わりにCSRという言葉を用いることができてしまいますが、厳密にはCSRとSPAは別の用語です。)
...
**意味: 同一のルート(ページ)内でSSR/SSGのようにレンダリングするコンポーネントを両立するレンダリングモデル**

PPRはNext.jsで登場した新しいレンダリング手法です。そしてレンダリングの焦点はサーバーサイド(ビルド時も含む)にあります。
なお、PPRという概念・機能はNext.jsに限定したものではなく、以下の記事では Cloudflare Workersと従来のReactのSSRフレームワーク で Reactの prerender() APIを使ってPPRを実装する方法が述べられています。
https://sunilpai.dev/posts/ppr-for-everyone/

しかしここでは、Next.jsのドキュメントにならって説明します。

PPRを文字通り直訳すると部分的(Partial)な事前レンダリング(Prerendering ≒ SSG)となりますが、公式のドキュメントでは、**staticなコンポーネントとdynamicなコンポーネントを同一ルート(ページ)内に共存させることを実現する機能**であると説明されています。
