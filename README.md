...
[^MPA]:Multi-page application。遷移時に新しい HTML ファイルが送られ、画面全体が書き換わるようなアプリケーションのこと。

[^CSROnly]:サイト訪問時も画面遷移時もCSRで行うような、クライアントサイドでのみ完全に動作するアプリケーション。サーバーサイドランタイムを持たず、build時にコンポーネントをレンダーしてコンテンツを含むHTMLを生成するわけではない。本記事で述べている、SPAの用例うちの一つに該当する。

[^SPA]: ここでは、サーバーサイドでレスポンスが返却される前に事前にレンダリングを全く行わない CSR のみのアプリケーションのこと
