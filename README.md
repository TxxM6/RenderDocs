...
CSRやSSRは、初回表示時や画面遷移時などのページを表示するタイミングに、**ページ　および　コンテンツをどのようにを生成、表示するか**という技術的な「レンダリングモデル」に焦点を当てた用語です。あるいは「レンダリング技術」と呼ぶことができそうです。
...
SSR という言葉は CSR(あるいは　Single-page application(SPA))が登場したときに生まれた**レトロ二ム**[^レトロニム]でした。しかし現在では、CSR だけでは解決できない課題を補うために、**サーバーサイドを活用するモダンなレンダリングモデルを指してSSRと呼ぶようになりました**(上記2の意味)。このパターンは、元々 「**SSR with Hydration**」 と呼ばれていたもので、サーバーサイドでの HTML 生成は初回表示時に限られ、その後のナビゲーションはソフトナビゲーションで行われます。よってもともとはCSRの対比の意味だったのが現在は、端的に「SSR with Hydrationで始まるSPA」　という意味に変容したことになります(SPAはここではソフトナビゲーションするアプリケーションのこと)。
...
その対策のために、サイト訪問時用に 別言語(Ruby など)で開発したサーバーを用意したり、ヘッドレスブラウザのようなものを挟み JavaScript を事前に実行する**ダイナミックレンダリング**などの手法により、コンテンツを含む HTML を生成していました。
...
現在、フロントエンド開発において ReactやVue.js といった 構築に適したJavaScriptフレームワークが普及し、さらに、 Next.js や Remix といった サーバーサイドでも JavaScript/React のコード　を実行できるフレームワークが主流となりました。これらのフレームワークはサーバーサイドにランタイムを設けることで、SPA の利点を享受しつつ、従来のSPAでは解決が難しかった課題の克服や最適化を実現しようとしています。
古典的なSSRのアプリケーションが主流だった時代とは異なり、モダンなSSRのフレームワークでは異なる言語との二重開発の問題が解消されており、非インタラクティブバージョンのページを素早く表示する手段としての意味を持つようにもなりました。
よって近年 SSR が言及される文脈は、モダンなフレームワークに関連する場合が多く、初期に **SSR with Hydration** と呼ばれていたものが、現在では**単に SSR** と呼ばれるようになり、その認識が広がってきています。
...
なお、CSRが 「常にクライアントサイドでのみで実行されるレンダリングモデル」 という意味で使われて続けている一方で、SSRを「常にサーバーサイドでのみ実行されるレンダリングモデル」と限定的に捉える主張はほとんど見られません。
この用語の焦点はHTML生成の方法にあることは間違いないようです。
...
Astro は [Static Site Generation(SSG)](#static-site-generation(ssg)(%E5%88%A5%E7%A7%B0%3A-pre-rendering)) に特化したフレームワークとして登場し、SPA に対抗し、従来の MPA(Ruby on Railsなど) のようにページ遷移を行いながらも、JavaScript 以外の言語との二重開発が不要であることを利点としています。
全てのページにおいて静的 HTML をビルド時に生成し、動的なコンテンツはクライアントサイドで外部 API からデータ取得してきて画面を更新すれば、 Node.js 等のサーバーサイドランタイムは不要です。
しかしOn-demand Rendering が追加され、新たにサーバーサイドランタイムを持つことで、これまでクライアントサイドで行っていた動的なコンテンツ生成 (CSR) をサーバーサイドで行い、コンテンツを含む HTML ファイルを配信できるようになりました。
...
SSR はサーバーサイドで HTML を事前に生成する技術を指すので、SC のレンダリングをそのまま SSR と呼ぶわけではありません。
...
:::message
RSC の SC は、RSC 以前のコンポーネント(つまり CC)と異なり、SSR 時にもハイドレーションされず、ソフトナビゲーション時もクライアントサイドで実行されません。
SCはCCとは異なりHooksやイベントハンドラなどの**インタラクティブ**な要素を持たないという制約があり、クライアントサイドでの状態管理や更新が不要となります。そのため、サーバーサイドやビルド時（以下まとめてサーバーサイド）にレンダリングが完結します。こうして得られた SC のレンダリング結果は、クライアントサイドの仮想 DOM（React ツリー）にそのまま適用できます。

また、SCがサーバーサイドで実行されることが約束されていることから、RSCでは新たな**データ取得方法**を実現しました。従来のSSRでは、初期表示に必要なデータはフレームワークが提供するデータ取得関数（例：Next.js の getServerSideProps や Remix の loader）を用いて、コンポーネントの外側で取得されていました。このデータ取得はソフトナビゲーション時にもサーバーサイドで実行され、取得したデータをPropsとしてコンポーネントに渡してレンダリングする、いわゆる「Fetch-then-render」という方式が用いられていました。

**従来: フレームワークによるデータ取得 → コンポーネントのレンダリング**

一方で RSC の場合、SC を非同期コンポーネントとすることで、各 SC が自らのレンダリング過程にサーバーサイドのデータ取得処理を組み込めるようになりました。つまり、RSCにおけるページのサーバーサイドのデータ取得はSCによって行われます。さらにストリーミング機能と組み合わせることで、データ取得が完了した部分から順次ブラウザでレンダリングを完了させることも可能です。
このように、RSC では SC を通じてデータ取得とレンダリングを統合することで、従来の「Fetch-then-render」とは異なる、効率的なサーバーサイドレンダリングのアプローチが実現されています。
:::
...
サーバーサイドランタイムの有無について定義されておらず、SSRやSSGと同時に成立し得る概念となっており、CSR/SSR/SSGを、SPAを構築するレンダリング技術であるという解釈することができます。
...
1つ目の意味同様ソフトナビゲーションで画面遷移しますが、**初回表示時には空のHTMLがされるような**、クライアントサイドでのみ実行するパターンのことを指します。よって、サーバーサイドのランタイムを持つ場合はSPAではないとする考え方もされているようです。
