...
- **Single-Page Application(SPA)**
  ...
  - 2. ソフトナビゲーションで画面遷移しクライアントサイドのみでデータ取得・コンテンツ生成を行う(SSR/SSG と排他的関係の概念)
...
1. **ソフトナビゲーションを行うアプリケーション全般(SSR/SSG と共存する概念)**
2. **ソフトナビゲーションを行いクライアントサイドのみで動作するアプリケーション(SSR/SSG と排他的関係の概念)**

Single-Page Application（SPA）は、Webアプリケーションのアーキテクチャを指す用語です。ただし、文献によっては、Webサイトの通信方式を指すものとして解釈される場合もあります。
筆者が観測する限り大きく分けて二つの意味で使用されているようですが、いずれの場合もソフトナビゲーションで画面遷移する点では共通しているようです。
CSR/SSR/SSGを、SPAを構築するレンダリング技術であるという解釈がある一方で、 SPAとSSRを対比させるケースも見られます。

### 1つ目の意味: ソフトナビゲーションを行うアプリケーション全般(SSR/SSG と共存する概念)
初回アクセス時を除き、**サーバーから新たにHTMLを送信せずに動作するアプリケーション**を指します。
このSPAが指すアーキテクチャとは、1つのHTMLページを最初に読み込み、以降のページ遷移では新しいHTMLページをサーバーから取得するのではなく、JavaScriptを使用して画面の一部を動的に更新する仕組みを指します。これにより、画面遷移があたかも1つのページ内で行われているように見えます。この仕組みはページ遷移ごとにHTMLファイルをサーバーから取得する**MPA（Multi-Page Application）[^MPA]との対比**として解釈が可能です。
...
### 2つ目の意味:　ソフトナビゲーションを行いクライアントサイドのみで動作するアプリケーション(SSR/SSG と排他的関係の概念)

[CSR](#client-side-rendering(csr))(クライアントサイドでのみ実行されるレンダリングモデル の意味)によって実現されます。
...
### SPA、SSR、SSG
SPAとSSRが対比として使われる用例では、ソフトナビゲーションをするアプリケーション同士の比較という意味があるようです。
この場合、SPAの意味を 2つ目の意味の **サーバーサイドでレンダリングをしない クライアントサイドのみで動作するアプリケーション**(CSR Only) の意味合いで捉えており、SSR、SSGとの差異は、初回アクセス時に配信するコンテンツ(HTML)のレンダリング方法の違いになります。
...
:::message alert
Reactなどの場合、SSR、SSGではブラウザ上でもコンポーネåント(CC)の実行(ハイドレーション)が必要です。
:::
