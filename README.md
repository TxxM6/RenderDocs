...
現在でもCSRという用語は、このような一貫してクライアントサイドで処理が実行されるパターンを示す言葉として使われ続けています。例えば、Next.js  Pages Routerの[Client-side Rendering (CSR)](https://nextjs.org/docs/pages/building-your-application/rendering/client-side-rendering)のページでも、ブラウザは最小限のHTML(minimal HTML)をダウンロードし、ブラウザのJavaScriptがDOMを更新するパターンをCSRであるとしています。[^NextjsにおけるCSR]
