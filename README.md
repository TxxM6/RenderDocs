...
**現在の SSR の主流の意味. (サーバーサイド JavaScript ランタイムを用意して) 毎ページリクエスト時に HTML ファイル を生成すること、あるいはそのレンダリングパターン**
...
また、モダンな SSR フレームワークは過去のものとは生い立ちや性質が異なるため、SSR という言葉で肝となる課題感も変わっています。
現在の SSR の意味は、敢えてサーバーサイド JavaScript ランタイムを用意するというニュアンスが加わっています。
...
CSR(SPA)台頭の時点では当然、サーバーサイドでコンテンツ(HTML)を生成することが一般的であり、敢えてクライアントサイドでコンテンツをレンダリングするという点がこれらの用語の焦点になっていました。
...
**現在の SSR の主流の意味(再掲). (サーバーサイド JavaScript ランタイムを用意して) 毎ページリクエスト時に HTML ファイル を生成すること、あるいはそのレンダリングパターン**
...
現在 SSR について言及される場面は主にこれらのフレームワークの文脈であることが多く、初期に SSR with Hydration と呼ばれているものが単に SSR と呼ばれるようになりました。

SSR をする目的は、オンデマンドにブラウザに届く前の段階でコンテンツを含む HTML ファイルを生成することを動的なページで実現することです。
元々 SSR という言葉が誕生した CSR(SPA)台頭の時点では、本来はサーバーサイドでコンテンツ(HTML)を生成していたところをクライアントサイドで実行するようになったという文脈がありました。しかし、現在の SSR という用語は、むしろ**本来はクライアントで動くコード**をサイト訪問時だけ、**敢えてサーバーサイドでレンダリングする**という文脈の言葉に変わってきたと考えることができそうです。

:::message
Astro と SSR
MPA のフレームワークである Astro でも SSR(Astro 的にはオンデマンドレンダリング)という言葉が使われています。Astro のドキュメントでの意味は、ページリクエスト時に HTML をサーバーサイドで生成すること としています。
Astro はもともと Static Site Generation に特化して登場したフレームワークで、SPA に対抗して、Rails などと同様 MPA 的な遷移をしながらも JavaScript 以外の言語との二重開発をする必要がないことを利点としていました。
SSG の静的ページのみで良ければ、build 時に生成した静的 HTML を配信して、外部 API とのやり取りはブラウザで行えば node.js 等のランタイムは不要でしたが、後から SSR ができるようになりました。

この Astro の SSR ににおいても、本来持たなかったサーバーサイドランタイムを持つことで、オンデマンドにサーバーサイドでコンテンツ入りの HTML ファイルを生成するためのものだということができそうです。
:::

##### MPA と SSR

SSR という用語は 現在も、NextJs の PagesRouter のように同一画面に対して、サイト訪問時には SSR、ページ遷移時には CSR という風に、コードの実行場所を切り替えるフレームワーク以外でも使われてます。

Astro は現在 SSR ができるフレームワークとなっていますが、MPA のフレームワークです。
Astro はもともと Static Site Generation に特化して登場したフレームワークで、SPA に対抗して、Rails などと同様 MPA 的な遷移をします。
SSG をする場合は、build 時に生成した静的 HTML を配信すればよいので node.js 等のランタイムは不要でしたが、後から SSR(Astro 的にはオンデマンドレンダリング)ができるようになりました。

Astro では、Server-Side Rendering(on-demand rendering)の意味は、ページリクエスト時に HTML をサーバーサイドで生成することとしています。

:::message
CSR を行わない JavaScript のフレームワークである Astro では、Server-Side Rendering(on-demand rendering)の意味は、ページリクエスト時に HTML をサーバーサイドで生成することとしています。
:::

### Astro

Astro はもともと Static Site Generator (SSG 用)として登場したフレームワークです。SSG をする場合は、不要で build 時に生成した静的 HTML を配信すればよいので node.js 等のランタイムは不要でした。そして、ページ遷移では SPA 以前の Rails のようなフレームワーク動揺、CSR ではなく MPA 的に行います。
そんな Asrto でも後から SSR(オンデマンドレンダリング)ができるようになりました。
Astro 内の SSR の説明では以下の意味合いになりそうです。

リクエスト時にサーバー上で HTML ページを生成し、結果(の HTML ファイル)をクライアントに送信すること

よって細かいのですが、SSR は MPA のフレームワークでも使われる概念であります。
しかし、重要である点は、Astro が JavaScript 以外の言語との二重開発をする必要がない MPA フレームワークである点を押し出していること、そして、SSR の機能は、本来持たなかったサーバーサイドランタイムを持つことで、オンデマンドにサーバーサイドでコンテンツ入りの HTML ファイルを生成していることです。SSR 以外の方法で Astro が動的なページを作る場合は、その処理はクライアント側で行う必要がありました。
