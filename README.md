...
- **Client-Side Rendering(CSR)**
  - 1. クライアントサイドのみでデータ取得・コンテンツ生成・DOM更新を行うレンダリングモデル
  - 2. クライアントサイドでデータ取得・コンテンツ生成・DOM更新を行うこと
- **Server-Side Rendering(SSR)**
  - 1. リクエスト時にサーバーサイドでHTMLを生成するレンダリングモデル全般
  - 2. 初回表示のリクエスト時にはHTMLをサーバーサイドで生成し、クライアントサイドでハイドレーションを行うレンダリングモデル
...
CSRやSSRは、初回表示時や画面遷移時などのページを表示するタイミングに、**ページ　および　コンテンツをどのようにを生成、表示するか**という技術的な「レンダリングモデル」に焦点を当てた用語です。
ページを表示する際には、データの取得やコンテンツ生成といったプロセスがあり、それらをHTMLとして出力したり、DOMを操作することで画面に反映されます。
これらの用語の区別は、簡潔に言えば、このプロセスが実行される場所・タイミングが、クライアントサイドであればCSR、サーバーサイドランタイムであればSSR、ビルド時であればSSGであると言えるでしょう。

なお、アプリケーション全体でどのようにページの生成・表示するパターンに当てはまるか を指す意味でも使われますが、初回表示時・画面遷移時に区別をつけず、ページおよびコンテンツの生成・表示がどこでどのように実行されているのかを表す用語としても利用されます。
...
SPAは、アプリケーション全体の動作の仕組みや構成方法を指す用語です。ソフトナビゲーションという遷移方法によって単一のHTML内でページ遷移をする際、ページ全体を再読み込みせず、必要な部分だけを更新するアプリケーションのことを指します。
...
### 1つ目の意味: クライアントサイドのみでデータ取得・コンテンツ生成・DOM更新を行うレンダリングモデル

**従来のWebアプリケーション**では、初回表示時もページ遷移時も、サーバー側でデータ取得・ページ生成を行い、完成したHTMLをブラウザに送信し、画面全体を更新していました(MPA[^MPA])。

一方、**CSR**のアプリケーションでは初回表示時やハードナビゲーションの際には以下のような流れでページが表示されます: 
- コンテンツが空のHTMLレスポンスがサーバーから送信される
- ブラウザがJavaScriptを読み込み、その後、**データ取得・コンテンツ生成**を**クライアントサイド**で行い**DOMを更新する**
  
ページ遷移をソフトナビゲーションを行う場合は、新たにHTMLレスポンスを受け取ることはなく、同様の流れで**データ取得・コンテンツ生成**を行い、現在の画面から部分的にDOMを更新することができます。

現在でもCSRという用語は、このような一貫してクライアントサイドで処理が実行されるパターンを示す言葉として使われ続けています。例えば、Next.jsの[Client-side Rendering (CSR)](https://nextjs.org/docs/pages/building-your-application/rendering/client-side-rendering)のページでも、ブラウザは最小限のHTML(minimal HTML)をダウンロードし、ブラウザのJavaScriptがDOMを更新するパターンをCSRであるとしています。[^NextjsにおけるCSR]

:::message
文献によってはCSRの説明でソフトナビゲーションがセットであるような説明がされていることも多く、この場合はCSRで実現されたSPAと見分けがつきにくくなります。主要なCSRのアプリケーション用のフレームワークが同時にSPAを実現することも多いためではないかと考えています。
筆者はCSRはハードナビゲーションしても成り立つのに対して、SPAはハードナビゲーションと相容れない関係である点でも別の概念であると考えています。
:::
...
### 2つ目の意味: クライアントサイドでデータ取得・コンテンツ生成・DOM更新を行うこと
「初回表示時」や「ページ遷移時」など特定の場面で、**クライアントサイドでデータ取得をしコンテンツ生成およびDOMを更新すること**を指します。
初期表示時やハードナビゲーションでは、上述した空のHTMLレスポンスがサーバーから返却される工程が必要になります。しかし、ソフトナビゲーションであってもJavaScriptがロードされた後の**データ取得からDOMを更新して画面に表示するまでの過程**は共通して行われます。そして、この画面を表示する実行処理の部分を指してCSRと呼びます。
　　
...
また、以下の最近の記事でも古典的なレンダリングモデルもServer-Side Renderingであると解釈しています。(なお、この記事ではSSRとCSRは排他的な関係ではなく、例えばSSR with Hydration のパターンはSSRとCSRが合体した姿であると解釈しています。) 
...
1. **ソフトナビゲーションで画面遷移するアプリケーション全般(SSR/SSG と共存する概念)**
2. **クライアントサイドのみで動作するレンダリングモデル(SSR/SSG と排他的関係の概念)**

 Single-page applicarion(SPA)は、レンダリングモデルに関する用語で、筆者が観測する限り大きく分けて二つの意味で使用されているように見受けられます。
...
### 2つ目の意味:　ソフトナビゲーションで画面遷移しクライアントサイドのみでデータ取得・コンテンツ生成を行う(SSR/SSG と排他的関係の概念)
